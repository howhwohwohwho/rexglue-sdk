...
REXCVAR_DEFINE_BOOL(vulkan_log_debug_messages, true, "UI/Vulkan",
...
std::unique_ptr<VulkanInstance> VulkanInstance::Create(
...
#if REX_PLATFORM_MAC
  const MacOSVulkanRuntimePaths macos_runtime_paths =
      DetectMacOSVulkanRuntimePaths();
  ConfigureMacOSVulkanEnvironment(macos_runtime_paths);

  std::vector<std::filesystem::path> loader_candidates =
      macos_runtime_paths.loader_candidates;
  ...
  if (!loader_loaded) {
    ...
    for (const std::string& attempted_loader_path :
         attempted_loader_paths) {
  ...
  loader_loaded =
      vulkan_instance->loader_.Load(platform::lib_names::kVulkanLoader);

  if (!loader_loaded) {
    loader_loaded =
        vulkan_instance->loader_.Load("libvulkan.dylib");
  }

  if (!loader_loaded) {
    loader_loaded =
        vulkan_instance->loader_.Load("libMoltenVK.dylib");
  }

  if (!loader_loaded) {
    REXLOG_ERROR("Failed to load a Vulkan runtime on iOS");
    return nullptr;
  }
  ...
  if (!loader_loaded) {
    REXLOG_ERROR("Failed to load {}",
                 platform::lib_names::kVulkanLoader);
    return nullptr;
  }
#endif
...
#define XE_VULKAN_LOAD_LOADER_FUNCTION(name)                              \
  functions_loaded &=                                                     \
      (ifn.name = vulkan_instance->loader_.GetSymbol<PFN_##name>(#name)) \
          != nullptr;
...
#undef XE_VULKAN_LOAD_LOADER_FUNCTION
  ...
  functions_loaded &= (ifn.vkCreateInstance = PFN_vkCreateInstance(
                           ifn.vkGetInstanceProcAddr(
                               nullptr, "vkCreateInstance"))) != nullptr;
  ...
  // Available since Vulkan 1.1. If this is nullptr, it's a Vulkan 1.0
  // instance.

  ifn.vkEnumerateInstanceVersion =
      PFN_vkEnumerateInstanceVersion(ifn.vkGetInstanceProcAddr(
  ...
  if (vulkan_instance->api_version_ >=
      VK_MAKE_API_VERSION(0, 1, 1, 0)) {
    vulkan_instance->extensions_
        .ext_1_1_KHR_get_physical_device_properties2 = true;
  } else {
    // #60.
  ...
  while (true) {
    ...
    const VkResult
        get_supported_implementation_extension_count_result =
            ifn.vkEnumerateInstanceExtensionProperties(
                nullptr,
                &supported_implementation_extension_count,
                nullptr);

    if (get_supported_implementation_extension_count_result !=
            VK_SUCCESS &&
        get_supported_implementation_extension_count_result !=
            VK_INCOMPLETE) {
      REXLOG_WARN(
          "Failed to get the Vulkan instance extension count");
      return nullptr;
    }
    ...
    if (supported_implementation_extension_count) {
      ...
      const VkResult
          get_supported_implementation_extensions_result =
              ifn.vkEnumerateInstanceExtensionProperties(
                  nullptr,
                  &supported_implementation_extension_count,
                  supported_implementation_extensions.data());

      if (get_supported_implementation_extensions_result ==
          VK_INCOMPLETE) {
        continue;
      }
  ...
  // If enabled layers are not present, will disable all extensions
  // provided by the layers by truncating the enabled extension vector
  // to this size.
  ...
  // Name pointers from `requested_layers` will be used in the enabled
  // layer vector.
  ...
  if (!requested_layers.empty()) {
    ...
    // "The list of available layers may change at any time due to
    // actions outside of the Vulkan implementation"
    ...
    if (!available_layers.empty()) {
      ...
      for (const VkLayerProperties& available_layer :
           available_layers) {
        ...
        // "Because the list of available layers may change
        // externally between calls to
        // vkEnumerateInstanceExtensionProperties, two calls may
        // retrieve different results if a pLayerName is available
        // in one call but not in another."

        while (true) {
          uint32_t supported_layer_extension_count = 0;

          const VkResult
              get_supported_layer_extension_count_result =
                  ifn.vkEnumerateInstanceExtensionProperties(
                      nullptr,
                      &supported_layer_extension_count,
                      nullptr);

          if (get_supported_layer_extension_count_result !=
                  VK_SUCCESS &&
              get_supported_layer_extension_count_result !=
                  VK_INCOMPLETE) {
          ...
          if (supported_layer_extension_count) {
            supported_layer_extensions.resize(
                supported_layer_extension_count);

            const VkResult
                get_supported_layer_extensions_result =
                    ifn.vkEnumerateInstanceExtensionProperties(
                        available_layer.layerName,
                        &supported_layer_extension_count,
                        supported_layer_extensions.data());

            if (get_supported_layer_extensions_result ==
                VK_INCOMPLETE) {
              continue;
            }
        ...
        for (const VkExtensionProperties& supported_extension :
             supported_layer_extensions) {
          const auto requested_extension_it =
              requested_extensions.find(
                  supported_extension.extensionName);

          if (requested_extension_it ==
          ...
          // Don't add the extension to the enabled vector multiple
          // times if provided by the implementation itself or by
          // another layer.
        ...
        if (!*requested_layer_it->second) {
          enabled_layers.emplace_back(
              requested_layer_it->first.c_str());
  ...
  // VK_KHR_get_physical_device_properties2 is needed to get the
  // portability subset features.

  if (vulkan_instance->extensions_
          .ext_KHR_portability_enumeration &&
      vulkan_instance->extensions_
          .ext_1_1_KHR_get_physical_device_properties2) {
...
#undef XE_UI_VULKAN_FUNCTION_PROMOTED
  ...
  // Non-promoted extensions, and extensions promoted to a Vulkan
  // version not supported by the instance.

#define XE_UI_VULKAN_FUNCTION_PROMOTED(extension_name, core_name)       \
  functions_loaded &=                                                    \
      (ifn.core_name = PFN_##core_name(                                 \
           ifn.vkGetInstanceProcAddr(                                   \
               vulkan_instance->instance_, #extension_name))) !=       \
      nullptr;
...
#undef XE_UI_VULKAN_FUNCTION
  ...
  if (!functions_loaded) {
    REXLOG_ERROR(
        "Failed to get all Vulkan instance function pointers");
    return nullptr;
  }
  ...
  if (with_surface &&
      !VulkanPresenter::GetSurfaceTypesSupportedByInstance(
          vulkan_instance->extensions_)) {
    REXLOG_ERROR(
        "The Vulkan instance doesn't support surface types used by Xenia");
    return nullptr;
  }
  ...
  REXLOG_INFO(
      "Vulkan instance API version {}.{}.{}. Enabled layers and "
      "extensions:",
  ...
  for (uint32_t enabled_layer_index = 0;
    ...
    REXLOG_INFO(
        "* {}",
        instance_create_info
            .ppEnabledLayerNames[enabled_layer_index]);
  }
  ...
  for (uint32_t enabled_extension_index = 0;
    ...
    REXLOG_INFO(
        "* {}",
        instance_create_info
            .ppEnabledExtensionNames[enabled_extension_index]);
  }
  ...
  if (vulkan_instance->extensions_.ext_EXT_debug_utils &&
    ...
    // VUID-VkDebugUtilsMessengerCreateInfoEXT-messageSeverity-
    // requiredbitmask:
    ...
    if (debug_utils_messenger_create_info.messageSeverity) {
      ...
      if (debug_utils_messenger_create_result != VK_SUCCESS) {
        REXLOG_WARN(
            ...
            vk::to_string(
                vk::Result(debug_utils_messenger_create_result)));
      }
    }
  }
...
void VulkanInstance::EnumeratePhysicalDevices(
  ...
  while (true) {
    ...
    const VkResult get_physical_device_count_result =
        functions_.vkEnumeratePhysicalDevices(
            instance_,
    ...
    physical_devices_out.resize(
        get_physical_devices_result == VK_SUCCESS
            ? physical_device_count
            : 0);
...
VkBool32 VulkanInstance::DebugUtilsMessengerCallback(
  ...
  for (uint32_t queue_label_index = 0;
    ...
    log_str << "queue label "
            ...
            << ": "
            << callback_data
                   ->pQueueLabels[queue_label_index]
                   .pLabelName;
  }
  ...
  for (uint32_t cmd_buf_label_index = 0;
    ...
    log_str << "command buffer label "
            << cmd_buf_label_index
            << ": "
            << callback_data
                   ->pCmdBufLabels[cmd_buf_label_index]
                   .pLabelName;
  }

  for (uint32_t object_index = 0;
       object_index < callback_data->objectCount;
       ++object_index) {
    begin_annotation();
    ...
    // Lowercase hexadecimal digits in the handle to match the
    // default Vulkan debug utils messenger.

    log_str << "object "
            << object_index
            << ": "
            << vk::to_string(
                   vk::ObjectType(object_info.objectType))
            << " 0x"
            << std::hex
